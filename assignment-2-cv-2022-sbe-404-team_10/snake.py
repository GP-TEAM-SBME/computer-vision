...
#         self.gradientY = cv2.Sobel( self.gray, cv2.CV_64F, 0, 1, ksize=5 )
        ...
        self.binary = adaptive_thresholding(self.gray)
...
def Sobel(image):
    '''
    Apply a Sobel detector to a given image to extract edges in it.
    Args:
        image (np.array): the image to detect edges in.
    Returns:
        gradients_x (np.array): the gradient for every pixel along x direction.
        gradients_y (np.array): the gradient for every pixel along y direction.
    '''
    ...
    gradients_x = convolution(image, kernel_x)
    gradients_y = convolution(image, kernel_y)
...
def convolution(image, kernel):
    ...
    Args:
        image (np.array): the gray-scale image to be convolved.
        kernel (np.array): the kernel to apply at each image pixel.
    Returns:
        conv_image (np.array): the resulted image after convolution.
    '''
    ...
    conv_image = np.zeros_like(image, dtype=np.uint8)
        ...
        for j in range(w, image_w - w):
            ...
            for m in range(kernel_h):
                for n in range(kernel_w):
                    conv_sum += kernel[m][n] * image[i - h + m][j - w + n]

            conv_image[i][j] = conv_sum
            
    return conv_image


def adaptive_thresholding(img, threshold=15, window_coef=16 ):
